...
@anfitriao_router.get("/ativos", status_code=HTTP_200_OK)  
def get_anfitrioes_ativos(  
    page: int = Query(1, ge=1),  
    page_size: int = Query(10, ge=1, le=50),  
):  
    """  
    Retorna anfitriões com status 'ativo' + dados do usuário,  
    com paginação e média de avaliações (se existir).  
    """  
    offset = (page - 1) * page_size  
    limit = page_size  
  
    # Busca anfitriões ativos com join de usuários  
    url = (  
    ...
    # Por enquanto, rating_medio fica None (será mockado no front como 5)  
    # Quando tiver avaliações, você pode fazer um segundo request ou usar RPC  
    for item in data:  
        item["rating_medio"] = None  # mock no front  
